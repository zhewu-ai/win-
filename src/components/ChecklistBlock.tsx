...
interface Props {
  ...
  /** 嵌入 Tiptap 文档（checklist 节点视图）时传 true：跳过容器级模糊清理、启用空行回车退出块。 */
  embedded?: boolean;
  /** embedded 时最后一行空行回车触发：块外插入新段落（节点视图实现）。 */
  onExitBlock?: () => void;
}
...
export default function ChecklistBlock({
  ...
  const exitBlockRef = useRef(onExitBlock);
  exitBlockRef.current = onExitBlock;
  ...
  // 行级键处理：Enter 加新待办（最后一行空行回车在 embedded 下退出块）、
  // 空行 Backspace 删行、Arrow 上下移焦。返回 true 表示已消费。
  const handleRowKeyDown = useCallback(
    (e: KeyboardEvent, _editor: Editor, id: string): boolean => {
      ...
      if (e.key === "Enter") {
        ...
        // M16 embedded：块内最后一行空行回车 → 退出块插入新段落（而非再新建空行）
        if (embeddedRef.current) {
          const sorted = [...itemsRef.current].sort(
            (a, b) => a.sortOrder - b.sortOrder
          );
          const last = sorted[sorted.length - 1];
          if (last && last.id === id && item.text.trim() === "") {
            exitBlockRef.current?.();
            return true;
          }
        }
        addTodoAfter(id);
  ...
  // 独立模式（embedded=false）：失焦时清理空行（全空则保留一行）。嵌入式交给文档序列化，不做清理。
