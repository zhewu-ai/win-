        ...
        {loading ? (
          <div className="p-4 space-y-2">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="h-20 rounded-card bg-white/[0.06] animate-pulse"
              />
            ))}
          </div>
        ) : notes.length === 0 ? (
          ...
          <div className="max-w-2xl mx-auto p-4 space-y-1">
            {notes.map((note) => {
              ...
              return (
                <div
                  key={note.id}
                  className="relative flex items-center gap-3 rounded-card px-4 py-3 hover:bg-white/[0.045] transition-colors"
                >
                  <span
